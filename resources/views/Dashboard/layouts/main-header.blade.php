                                       ...
                                       aria-expanded="false">
                                        @if (App::getLocale() == 'ar')
                                            <span class="avatar country-Flag mr-0 align-self-center bg-transparent"><img
                                                    src="{{URL::asset('Dashboard/img/flags/FlagEgypt.png')}}" alt="img"></span>
                                            <strong
						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
                            ...
                            <div class="dropdown nav-item main-header-notification">
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    <div class="menu-header-content bg-primary text-right">
                                        <div class="d-flex">
                                           <p class="dropdown-title mb-1 tx-15 text-white font-weight-semibold">الاشعارات</p>
                                            <span class="badge badge-pill badge-warning mr-auto my-auto float-left"></span>
                                        </div>
                                        <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">  @if($user)
                                            <h5> <span class="badge-pill badge-warning"> لديك {{ $user->unreadNotifications->count() }}</span> إشعار غير مقروء</h5>
                                            @endif</p>
                                    </div>
                                    ...
                                    @if($user)
                                                ...
                                                @foreach($user->notifications as $notification)
                                                    <a href="{{ route('notifications.markAsRead', $notification->id) }}"
                                                       class="list-group-item list-group-item-action {{ $notification->read_at ? '' : 'bg-light' }}">
                                                        ...
                                                        🏷️ اسم الرحلة: {{ $notification->data['message'] }} <br>
                                                        📅 تاريخ الرحلة: {{ $notification->data['trip_date'] }} <br>
                                                        👤 تم الطلب بواسطة: {{ $notification->data['requested_by'] }} <br>
                                        ...
                                        @else
                                            <p class="text-center text-danger">⚠️ يجب تسجيل الدخول لعرض الإشعارات.</p>
                                        @endif
                                    </div>

                                    <div class="dropdown-footer">
                                        @if($user)
                                            <a href="{{ route('notifications.markAllAsRead') }}" class="btn btn-sm btn-secondary">
                                                ✔️ قراءة الكل
                                            </a>
                                        @endif
                                    </div>
...
<!-- /main-header -->
    <script>
        ...
        $(".mark-as-read").click(function (e) {
            ...
            $.get(url, function () {
                location.reload(); // إعادة تحميل الصفحة بعد القراءة
            });
        });

        $("#markAllAsRead").click(function (e) {
        e.preventDefault();
        $.get("{{ route('notifications.markAllAsRead') }}", function () {
        location.reload();
    });
    });
    });
